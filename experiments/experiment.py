...
# Default input is experiments.json, use cli arg if given
...
try:
    ...
    data_path = experiment_data['imagefolder_path']
...
for parameters in experiment_data['experiments']:
    ...
    target_num_of_classes = len(class_names)

    device = torch.device("cuda:0" if torch.cuda.is_available() else 'cpu')
    print('Cuda GPU detected' if torch.cuda.is_available() else 'No Cuda GPU detected')
    ...
    # Save model type to log to MLflow    
    ...
    # Log the pretrained model as artifact
